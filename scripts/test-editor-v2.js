...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const injected = [
      ...
      // U. 日期占位符 + 字体
      "  const uEl = elementByKey(selectedElement);",
      "  uEl.text = '生产:{今天} 保质:{明天}';",
      "  uEl.font = '楷体';",
      "  renderPreview(); await sleep(100);",
      "  const uNode = [...document.querySelectorAll('.el-custom')].pop();",
      "  const dNow = new Date();",
      "  const pad2 = (n) => String(n).padStart(2, '0');",
      "  const todayStr = dNow.getFullYear() + '-' + pad2(dNow.getMonth() + 1) + '-' + pad2(dNow.getDate());",
      "  const dTmr = new Date(dNow.getTime() + 86400000);",
      "  const tmrStr = dTmr.getFullYear() + '-' + pad2(dTmr.getMonth() + 1) + '-' + pad2(dTmr.getDate());",
      "  const uText = uNode.textContent;",
      "  log('U text=' + uText);",
      "  if (!uText.includes(todayStr) || !uText.includes(tmrStr)) throw new Error('placeholders not resolved: ' + uText);",
      "  if (!uNode.style.fontFamily.includes('楷体')) throw new Error('font not applied: ' + uNode.style.fontFamily);",
      "  const htmlU = LabelRender.buildPrintHtml([{ name: 'A', sku: 'S', price: 1, barcodeSvg: '', barcodeText: '' }], { labelSize: '60x40', custom: [{ text: '生产:{今天}', x: 2, y: 2, w: 30, fontSize: 3, font: '楷体' }] }, false);",
      "  if (!htmlU.includes('font-family:' + String.fromCharCode(39) + '楷体' + String.fromCharCode(39))) throw new Error('print font css missing');",
      "  if (!htmlU.includes(todayStr)) throw new Error('print placeholder not resolved');",
      "  uEl.font = '';",
      "  log('U placeholder/font ok');",
      // V. 快速打印（替换 sendPrint 拦截打印，验证参数与历史记录）
      "  const origPrint = sendPrint;",
      "  let quickCalled = null;",
      "  sendPrint = (payload, opts) => { quickCalled = { n: payload.length, opts }; return Promise.resolve({ ok: true }); };",
      "  const firstRowCheck = document.querySelector('#productBody .row-check');",
      "  if (!firstRowCheck) throw new Error('no product rows for quick print');",
      "  firstRowCheck.checked = true;",
      "  $('btnQuickPrint').click();",
      "  await sleep(400);",
      "  sendPrint = origPrint;",
      "  if (!quickCalled || quickCalled.n !== 1) throw new Error('quickPrint not called: ' + JSON.stringify(quickCalled));",
      "  if (!quickCalled.opts.settings || !quickCalled.opts.printOptions) throw new Error('quickPrint missing settings/options');",
      "  const histV = await window.api.getPrintHistory();",
      "  if (histV.history.length < 2) throw new Error('quickPrint history not recorded');",
      "  log('V quick print ok (history=' + histV.history.length + ')');",
      // W. 行内改价 + 表单 Enter 连续录入
      "  flushAutoSave(); $('btnPrintCancel').click(); await sleep(100);",
      "  const chipRow = [...document.querySelectorAll('#productBody tr')].find((tr) => tr.textContent.includes('薯片'));",
      "  if (!chipRow) throw new Error('薯片 row missing');",
      "  const priceTd = chipRow.querySelector('td.col-price');",
      "  priceTd.dispatchEvent(new MouseEvent('dblclick', { bubbles: true }));",
      "  await sleep(80);",
      "  const wInp = priceTd.querySelector('input');",
      "  if (!wInp) throw new Error('inline price input missing');",
      "  wInp.value = '9.9';",
      "  wInp.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));",
      "  await sleep(300);",
      "  const chip = products.find((x) => x.name === '薯片');",
      "  log('W chip price=' + chip.price);",
      "  if (Number(chip.price) !== 9.9) throw new Error('inline price edit failed: ' + chip.price);",
      "  $('fName').value = '键盘录入商品';",
      "  $('fSku').value = 'T-KB';",
      "  $('fBarcodeType').value = 'CODE128';",
      "  $('fBarcode').value = 'KB1';",
      "  $('fPrice').value = '3.5';",
      "  $('fName').dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));",
      "  await sleep(300);",
      "  const kb = products.find((x) => x.name === '键盘录入商品');",
      "  if (!kb) throw new Error('Enter submit failed');",
      "  const focused = document.activeElement === $('fName');",
      "  log('W enter submit ok, name focused=' + focused);",
      "  if (!focused) throw new Error('focus should return to name input');",
